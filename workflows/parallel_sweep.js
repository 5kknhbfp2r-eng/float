...
A) CARRY-FORWARD: PYTHONUTF8=1 ${PY} -c "import csv; rs=[(r['as_of'],r['float_M'],r['os_M']) for r in csv.DictReader(open('${ROOT}/float_is.csv')) if r['ticker']=='TICKER' and r['as_of']<'AS_OF']; print(rs[-3:])". If a prior date exists, run the dossier for AS_OF and use fg.list_filings('TICKER','AS_OF') to check for any NEW O/S-relevant filing (periodic/424B/S-1/8-K-issuance/split) between the prior date and AS_OF; if NONE and current O/S unchanged -> record the SAME float_M & os_M, NOTE="carried from <prior date>; no intervening O/S change". Else derive fresh.
